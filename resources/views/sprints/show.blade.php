@props(['user_identifier' => null, 'project_by_user_identifier' => null, 'sprint_nr' => null, 'user' => Auth::user()])
@php
...
<html lang="nl" class="h-full">
   <head>
      ...
      <title>{{ $project->title }} - Sprint {{ $sprint->sprint_nr }}</title>
   ...
   <body class="h-full flex flex-col min-h-screen">
      <x-header subtitle="{{ $profile_user->full_name }}'s Projectkaarten > {{ $project->title }} >" subtitleurl="{{ route('home') }}/{{ $profile_user->identifier }}/{{ $project->project_by_student }}" title="Sprint {{ $sprint->sprint_nr }}" />
